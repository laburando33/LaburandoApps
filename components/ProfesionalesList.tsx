  ...
  useEffect(() => {
    const fetchProfesionales = async () => {
      try {
        ...
        setProfesionales((data as Professional[]) || [])
      } catch (error) {
...
function ProfesionalCard({ profesional }: { profesional: Professional }) {
  return (
    <div className="bg-card text-card-foreground rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <div className="p-6">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-lg font-semibold">{profesional.profile.full_name}</h3>
            <p className="text-sm text-muted-foreground mt-1">{profesional.services.join(", ")}</p>
          </div>
            ...
            <div className="flex items-center">
              {[...Array(5)].map((_, index) => (
                <Star
                  key={index}
                  className={`h-5 w-5 ${
                    index < Math.floor(profesional.rating) ? "text-yellow-400 fill-current" : "text-gray-300"
                  }`}
                />
              ))}
            </div>
            <span className="ml-2 text-sm text-muted-foreground">({profesional.total_reviews} reseñas)</span>
          </div>
        </div>
